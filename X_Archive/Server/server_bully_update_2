...
def discover_servers():
    ...
    try:
        for port in range(6001, 7000):
            try:
                client_socket.sendto(DISCOVERY_MESSAGE_SERVER.encode(FORMAT), (MULTICAST_GROUP, port))
            except Exception:
                continue
...
def handle_message(message, addr):
    ...
    if message.startswith("DISCOVER_SERVER"):
        ...
        heartbeat_tracker[addr] = time.time()
        ...
        print(f"[COORDINATOR] Neuer Koordinator: {coordinator}")
...
def start_election():
    ...
    higher_rank_servers = [server for server in servers.values() if int(server.split(",")[0].split(":")[1], 16) > server_rank]
    if not higher_rank_servers:
        coordinator = server_id
        print(f"[ELECTION] Neuer Koordinator: {coordinator}")
        for server in servers.values():
            try:
                server_details = server.split(",")
                server_ip = server_details[1].split(":")[1]
                server_port = int(server_details[2].split(":")[1])
                udp_server.sendto(f"COORDINATOR|{server_id}".encode(FORMAT), (server_ip, server_port))
                print(f"[COORDINATOR] Nachricht gesendet an {server_ip}:{server_port}")
            except Exception as e:
        ...
        for server in higher_rank_servers:
            try:
                ...
                server_ip = server_details[1].split(":")[1]
                server_port = int(server_details[2].split(":")[1])
                udp_server.sendto(f"ELECTION|{server_rank}".encode(FORMAT), (server_ip, server_port))
                print(f"[ELECTION] Wahl-Nachricht gesendet an {server_ip}:{server_port}")
            except Exception as e:
...
def broadcast_coordinator():
    ...
    for server in servers:  # *NEU*
        try:
            server_1 = filter_server_ip_port(server)
            # Beispielcode für TCP-Broadcast
            udp_server_zwei.sendto(f"COORDINATOR|{server_id}".encode(FORMAT),server_1)  # *NEU*
            print(f"[COORDINATOR] Server {server} informiert.")  # *NEU*
...
if __name__ == "__main__":
    ...
    if not coordinator:
        election_thread = threading.Thread(target=start_election)
